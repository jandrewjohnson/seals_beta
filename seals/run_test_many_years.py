...
if __name__ == '__main__':
    ...
    p.scenario_definitions_path = os.path.join(p.input_dir, 'many_years_scenario_defininitions.csv')

    # Set defaults and generate the scenario_definitions.csv if it doesn't exist.
    if not hb.path_exists(p.scenario_definitions_path):

        seals_utils.set_attributes_to_dynamic_many_year_default(p) # Default option
